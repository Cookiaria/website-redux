document.addEventListener('DOMContentLoaded', function () {
    ...
    async function handleLoadingBar(progress, forceShow = false) {
        // Ensure the loading bar element exists
        const loadingBar = document.getElementById('loadingbar');
        if (!loadingBar) {
        ...
        // Debug logging
        console.log(`Setting loading to ${progress}%`, { forceShow });
        
        return new Promise(resolve => {
            // Always update the internal state
            window.loadingBar.isLoading = progress > 0;
    ...
    async function loadContent(url, isPopState = false) {
        ...
        let loadingTimeout = setTimeout(async () => {
            await handleLoadingBar(30, true);
        }, 500);
    ...
    // Replace the existing assignment with:
    Object.assign(window.loadingBar, {
        handle: handleLoadingBar,
        isLoading: false
    });
    console.log('Switcher fully initialized', window.loadingBar);
});